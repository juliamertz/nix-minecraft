  ...
  in
    stdenvNoCC.mkDerivation (
      finalAttrs:
        ...
        // (lib.optionalAttrs (packHash != null) {
          outputHashMode = "recursive";
          outputHashAlgo = "sha256";
          outputHash = packHash;
        })
        // (removeAttrs args ["bootstrapUrl"])
    );

  # Fetch function that prepares parameters and calls the build function
  fetchPackwizModpack = {
    ...
    toml =
      if !srcNull
      then builtins.fromTOML (builtins.readFile (src + "/pack.toml"))
      else null;
    pname =
      args.pname or (
        if !srcNull
